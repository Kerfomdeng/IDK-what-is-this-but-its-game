...
class Player(GameSprite):
    ...
    def update(self):
        ...
        if keys[K_LEFT] and self.rect.x > -40 and n!=3:
            self.rect.x -= self.speed
            self.s=2
        if keys[K_RIGHT] and self.rect.x < win_width-60 and n !=1 :
            self.rect.x += self.speed
            self.s=3
        if keys[K_UP] and self.rect.y > -30:
            self.rect.y -= self.speed
            self.s=1
        if keys[K_DOWN] and self.rect.y < win_height-60 and n !=2:
            self.rect.y += self.speed
            self.s=0
...
walls1=sprite.Group()
w1=Wall(15,700,785,0,0,0,0)
w2=Wall(800,15,0,575,0,0,0)
w3=Wall(15,700,10,0,0,0,0)
walls1.add(w1,w2,w3)
    ...
    for e in event.get():
        ...
        if e.type == MOUSEBUTTONDOWN:
            ...
            if pos >= (20,10) and pos <= (130, 60):
                game='stop'
    if sprite.collide_rect(geroy,w1):
        n=1           
    elif sprite.collide_rect(geroy,w2):
        n=2
    elif sprite.collide_rect(geroy,w3):
        n=3    
    else:
        n=0
    if game == "start":
